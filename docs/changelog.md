...
- **Comprehensive Image Upscaling System**: Multi-backend Real-ESRGAN upscaling with `--upscale` flag
  - **Core ML Backend (macOS)**: Neural Engine acceleration with 2x and 4x native support
    - 2x: RealESRGAN_x2plus.mlpackage
    - 4x: RealESRGAN_x4plus_anime_6B.mlpackage (optimized for anime/manga)
    - Fallback URL system for reliable model downloads
    - Zero CPU/GPU usage during inference (hardware-accelerated)
  - **PyTorch Backend (Linux/Windows)**: CUDA/CPU support with 2x and 4x models
    - 2x: RealESRGAN_x2plus.pth
    - 4x: realesr-general-x4v3.pth
    - Three-tier retry strategy for memory errors
  - **NCNN Backend (macOS legacy)**: Vulkan-based alternative for older Macs
  - Tile-based processing (256×256 tiles) with weighted blending and feathering
  - SHA256-based marker system to skip already upscaled images
  - Graceful shutdown handling with threading events

### Security Enhancements

- **SHA256 Validation for All Backends**: Automatic integrity checks for downloaded models
...
- Automatic deletion of corrupted downloads
- Protection against malicious or corrupted model files
- Fallback URL system for Core ML with automatic retry on failure

### Platform Support

- **macOS ARM64 Build**: Native support for Apple Silicon (M1/M2/M3/M4)
  - Optimized Core ML upscaling with Neural Engine
  - Automatic quarantine attribute removal (`FIRST_RUN.command`)
  - Pre-configured for seamless execution without Gatekeeper issues
- **Windows x64/x86 Builds**: Full PyTorch backend support with CUDA acceleration
- **Linux Support**: PyTorch backend with CUDA/CPU support

### CI/CD Improvements

- Refactored GitHub Actions workflow for multi-platform builds
  - Separate build jobs for Windows (x64/x86) and macOS (ARM64)
  - Unified release job with artifact aggregation
  - Automated SHA-256 hash generation for release binaries
  - Fixed PowerShell output syntax for proper hash reporting
- Added Claude Code integration workflow

### Documentation

- **CLAUDE.md**: Comprehensive developer documentation (334 lines)
  - Architecture overview and pipeline explanation
  - Backend-specific implementation details
  - Model URLs, SHA256 hashes, and technical specifications
...
- **22 files changed**: +1963 additions, -282 deletions
- **New modules**:
  - `upscale.py` (313 lines): PyTorch backend implementation
  - `upscale_coreml.py` (473 lines): Core ML backend with Neural Engine
  - `upscale_ncnn.py` (348 lines): NCNN Vulkan backend
- **Modified core**:
  - `format/base.py`: Upscaling integration (+90 lines)
...
- Maintains full compatibility with original features and APIs

### Bug Fixes

- Fixed PowerShell output syntax in GitHub Actions for Windows builds
- Corrected repository name in cancel-workflow-action configuration
- Fixed Core ML model extraction for proper `.mlpackage` structure
